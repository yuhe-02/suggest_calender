...
function findAvailableSlots(busySlots: any[], timeMin: string, timeMax: string, durationMinutes: number) {
    ...
    while (checkTime < new Date(timeMax)) {
        ...
        const nextBusySlot = busyTimes.find(slot => slot.start < dayEndTime && slot.end > checkTime);
        ...
        if (freeUntil.getTime() - checkTime.getTime() >= durationMinutes * 60 * 1000) {
...
export async function POST(request: Request) {
    ...
    if (!(session as any)?.accessToken) {
        return NextResponse.json({ error: 'Not authenticated or access token is missing' }, { status: 401 });
    }

    const { attendees, startDate, endDate, duration } = await request.json();
    const accessToken = (session as any).accessToken;

    const allAttendees = [session.user?.email, ...attendees.split(',').filter(Boolean)];
    ...
    try {
        ...
        console.log(
            JSON.stringify(freeBusyResponse.data, null, 2)
        );
        ...
        const allBusySlots = Object.values(calendars).flatMap(cal => cal.busy || []);
        
        const mergedBusySlots = allBusySlots.reduce((acc: any[], slot) => {
        ...
        const availableSlots = findAvailableSlots(mergedBusySlots, timeMin, timeMax, duration);

        const formattedSlots = availableSlots.map(slot => slot.toLocaleString('ja-JP', { dateStyle: 'medium', timeStyle: 'short' }));
