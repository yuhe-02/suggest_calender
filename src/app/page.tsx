  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/suggest", {
        ...
        body: JSON.stringify({
          attendees: selectedCalendars.join(","),
          startDate,
          endDate,
          duration,
        }),
      ...
      if (data.suggestions.length === 0) {
        setError(
          "No available slots found, but you can see the schedules below.",
        );
      }
    } catch (err: any) {
      setError(err.message);
        ...
        {session ? (
          <div className="w-full">
            ...
            <form
              ...
              className="flex flex-col gap-4 bg-black/[.05] dark:bg-white/[.06] p-6 rounded-lg"
            >
              <div>
                ...
                <Select
                  ...
                  value={options.filter((opt) =>
                    selectedCalendars.includes(opt.value),
                  )}
                  onChange={(selected) =>
                    setSelectedCalendars(selected.map((opt: any) => opt.value))
                  }
                />
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="rounded-full bg-foreground text-background gap-2 hover:bg-[#383838] dark:hover:bg-[#ccc] font-medium text-sm h-10 px-5 disabled:bg-gray-400"
              >
